...
def process_voice_messages(file_path):
    ...
    # extract the emotion from the response text using regular expressions
    pattern = r'\[(.*?)\]'
    match = re.search(pattern, emotions)
    if match:
...
def execute_action(mood):
    if mood:
        if 'Felicità' in mood:
            ser.write(b'1') # Invia il comando
            print("1")
        elif 'Tristezza' in mood:
