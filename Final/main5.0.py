...
def detect_language(text):
    response = openai.Completion.create(
        ...
        prompt=f"Dato un testo come input, Voglio che tu mi risponda con la lingua del testo in output\n\n{text}?",
        temperature=0,
        max_tokens=64,
        top_p=1.0,
        frequency_penalty=0.0,
...
def translate_response(response_text, language):
    ...
    translation = translator.translate(response_text, dest=language).text
    return translation
    ...
    try:
        ...
        with open(file_path, 'wb') as f:
            ...
            try:
                ...
                translated_response = translate_response(emotion, language)
                context.bot.send_message(chat_id=update.effective_chat.id, text=translated_response)
...
def echo(update, context):
    ...
    execute_action(moods)  # Invia il comando all'ESP32 in base allo stato d'animo
    translated_response = translate_response(response_text, language)
    context.bot.send_message(chat_id=update.effective_chat.id, text=translated_response)
    print(f"Stato d'animo: {moods}")
