...
def echo(update, context):
    ...
    # Process the emotions and send the response to the user
    response_text = detect_emotions(message_text)
    moods = re.findall(r'\[(.*?)\]', response_text)
    ...
    try:
        # Try processing the text as a command to turn on or off the light
        command = re.findall(r'(accendi|accendimi|spegni|spegnimi) (la )?(luce)', message_text, flags=re.IGNORECASE)
        if len(command) > 0:
            if command[0][0].lower() in ['accendi', 'accendimi']:
                ser.write(b'6')
                print("Accendi la luce")
            elif command[0][0].lower() in ['spegni', 'spegnimi']:
                ser.write(b'7')
